...
function buddyboss_scripts_styles() {
	...
	if ( !is_admin() ) {
		...
		wp_enqueue_style( 'boss-main-global', $CSS_URL . '/main-global' . $class_sufix . '.css', array( 'fontawesome' ), '2.0.4', 'all' );
		...
		if ( isset( $_COOKIE[ 'switch_mode' ] ) && ( boss_get_option( 'boss_layout_switcher' ) ) ) {
			if ( $_COOKIE[ 'switch_mode' ] == 'mobile' ) {
				wp_enqueue_style( 'boss-main-mobile', $CSS_URL . '/main-mobile' . $class_sufix . '.css', array( 'fontawesome' ), '2.0.4', 'all' );
				$only_mobile = true;
			} else {
				wp_enqueue_style( 'boss-main-desktop', $CSS_URL . '/main-desktop' . $class_sufix . '.css', array( 'fontawesome' ), '2.0.4', 'screen and (min-width: 481px)' );
				wp_enqueue_style( 'boss-main-mobile', $CSS_URL . '/main-mobile' . $class_sufix . '.css', array( 'fontawesome' ), '2.0.4', 'screen and (max-width: 480px)' );
			}
			// Defaults
		} else {
			if ( is_phone() ) {
				...
				if ( boss_get_option( 'boss_layout_tablet' ) == 'desktop' ) {
					wp_enqueue_style( 'boss-main-desktop', $CSS_URL . '/main-desktop' . $class_sufix . '.css', array( 'fontawesome' ), '2.0.4', 'all' );
				} else {
					wp_enqueue_style( 'boss-main-mobile', $CSS_URL . '/main-mobile' . $class_sufix . '.css', array( 'fontawesome' ), '2.0.4', 'all' );
					$only_mobile = true;
				}
	...
	if ( !is_admin() ) {
		...
		if ( boss_get_option( 'boss_minified_js' ) ) {
			wp_register_script( 'boss-main-min', get_template_directory_uri() . '/js/compressed/boss-main-min.js', array( 'jquery' ), '2.0.4', true );
			...
			wp_register_script( 'buddyboss-main', get_template_directory_uri() . '/js/buddyboss.js', array( 'jquery', 'plupload', 'moxie' ), '2.0.4' );
...
add_filter( 'bp_disable_cover_image_uploads', 'boss_remove_bp_cover');
add_filter( 'bp_disable_group_cover_image_uploads', 'boss_remove_bp_cover');

if ( !function_exists( 'boss_remove_bp_cover' ) ):
/**
 * Remove default buddypress covers
 * @since Boss 2.0.2
 */

function boss_remove_bp_cover(){
    return true;
}
