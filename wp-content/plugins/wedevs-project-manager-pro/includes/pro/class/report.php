...
class CPM_Pro_Report {
    ...
    public static function co_worker_where( $where ) {
        ...
        if ( in_array( 'time', $data['action'] ) && $data['timemode'] == 'list' ) {
            ...
            $where .= " AND tl.post_type='task_list' AND tl.post_date >= '$from' AND tl.post_date <= '$to'";
            ...
            if ( $start_date_enable == 'on' ) {
                $where .= " AND tsk.post_type='task' AND tskmeta.meta_key = '_start' AND tskmeta.meta_value >= '$from' AND tskmeta.meta_value <= '$to'";
            } else {
                $where .= " AND tsk.post_type='task' AND tsk.post_date >= '$from' AND tsk.post_date <= '$to'";
            }
